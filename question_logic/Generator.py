    ...
    def Generate(self,quizData):
        ...
        completion = self.client.chat.completions.create(
            ...
            messages=[
                {
                    "role":"system",
                    "content":"The inputs will be in the following structure [Subject, Specification, Exam Board, Topic, Amount of Questions] use all information to create questions according to the subject, specification (A-level or GCSE for example), Exam board (AQA or Edexcel for example), Topic (What section of the subject they need, either a number or topic title). Then once all that is taken in, generate short fire questions max 15 words per question, following the amount that the user wants. Once done, return it in the format 'Question','Answer' so it is easier to store in a database. Under no circumstances generate Q'S and A's on non-real questions e.g. Game, movie references regardless If other parts of prompts are correct. Completely ignore and return that you cant complete the request. "
                },
                {
                    "role": "user",
